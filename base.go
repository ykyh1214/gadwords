...
import (
	...
	"git.zx-tech.net/yangk/gadwords/logger"
)
...
var (
	...
	adwordsLog                = logger.NewLogger(logger.NewDefaultWriter(&logger.DefaultWriterOption{Name: "adwords_"}))
)
...
func (a *Auth) request(serviceUrl ServiceUrl, action string, body interface{}) (respBody []byte, err error) {
	...
	adwordsLog.LogCalldepth(2, logger.LoggerLevel0Debug, "req:"+string(reqBody))
	...
	adwordsLog.LogCalldepth(2, logger.LoggerLevel0Debug, "res:"+string(respBody))
	// Added some logging/"poor man's" debugging to inspect outbound SOAP requests
	if level := os.Getenv("DEBUG"); level != "" {
		fmt.Printf("response ->\n%s\n", string(respBody))
	}
