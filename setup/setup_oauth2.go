...
var (
	googleConfigJSON = flag.String("credentials_json", "./credentialspwm.json", "API credentials from Google in JSON")
	newConfigJSON    = flag.String("new_config_json", "./configpwm.json", "API credentials & tokens for gads in JSON")
)
...
func Oauth2ConfigFromJSON(jsonKey []byte) (oac *oauth2.Config, err error) {
	...
	oac, err = google.ConfigFromJSON(jsonKey, "https://adwords.google.com/api/adwords", "https://www.googleapis.com/auth/analytics.readonly")
	if err == nil {
		return oac, err
	}
